...
function ResultsData() {
  ...
  useEffect(() => {
    fetch("http://localhost/soccer-dashboard/data_management/endpoints/getResults.php")
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div className="">
      {Object.entries(results).reverse().map(([week, dates]) => (
          ...
          {Object.entries(dates).map(([date, games]) => (
            <div key={date} className="mb-4">
              ...
              <ul>
                ...
                <li className="bg-slate-950 mb-4 p-2 rounded-md pl-2 pr-2 shadow-md shadow-gray-950">
                  <div className="flex flex-row justify-between items-center">
                    <img className="h-8 flex-.8 mr-2" src={game.home_team_logo} />
                    <p className={`flex-[4] truncate ${
                    game.winner === "HOME_TEAM"
                    ? "bg-green-600 rounded-lg" // gold
                    : console.log()
                    }`}>{game.home_team}</p>
                      <p className="text-lg flex-[2]">{game.home_score} - {game.away_score}</p>
                    <p className={`flex-[4] truncate ${
